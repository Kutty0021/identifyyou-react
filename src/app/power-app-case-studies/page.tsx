...
export const metadata = {
  ...
  description: "Microsoft Power Apps development and custom automation case studies.",
};

export default async function PowerAppCaseStudiesPage() {
  let cards: WPSolution[] = [];
  let pageTitle = "Power App Case Studies";
  
  try {
    const pageData = await getPageBySlug('power-app-case-studies');
    if (pageData) {
      pageTitle = pageData.title.rendered || "Power App Case Studies";
    }
    cards = await getPageCards('power-app-case-studies');
  } catch (err) {
    console.error("Error loading Power App case studies:", err);
  }
