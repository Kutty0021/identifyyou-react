...
export const metadata = {
  ...
  description: "Enterprise Resource Planning solutions tailored for your business operations.",
};

export default async function ErpSolutionsPage() {
  let cards: WPSolution[] = [];
  let pageTitle = "ERP Solutions";
  
  try {
    const pageData = await getPageBySlug('erp-solutions');
    if (pageData) {
      pageTitle = pageData.title.rendered || "ERP Solutions";
    }
    cards = await getPageCards('erp-solutions');
  } catch (err) {
    console.error("Error loading ERP solutions:", err);
  }
