...
export const metadata = {
  ...
  description: "Snowflake AI Data Cloud implementations.",
};

export default async function SnowflakeCaseStudiesPage() {
  let cards: WPSolution[] = [];
  let pageTitle = "Snowflake Case Studies";
  
  try {
    const pageData = await getPageBySlug('snowflake-case-studies');
    if (pageData) {
      pageTitle = pageData.title.rendered || "Snowflake Case Studies";
    }
    cards = await getPageCards('snowflake-case-studies');
  } catch (err) {
    console.error("Error loading Snowflake case studies:", err);
  }
