...
export default async function CaseStudyPage() {
  ...
  try {
    const pageData = await getPageBySlug('case-study');
    if (pageData) {
      pageTitle = pageData.title.rendered || "Case Studies";
    }
    cards = await getPageCards('case-study');
  } catch (err) {
    console.error("Error loading case studies page:", err);
  }
                  ...
                  <div className="bg-white border border-[#eeeeee] rounded-none overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:border-primary h-full flex flex-col">
                    ...
                    <div className="p-8 flex flex-col flex-grow">
                      ...
                      <div className="flex items-center text-primary font-bold mt-auto group-hover:text-secondary transition-colors">
                        <span className="uppercase tracking-wider text-sm">{card.buttonText || 'Case Studies'}</span>
