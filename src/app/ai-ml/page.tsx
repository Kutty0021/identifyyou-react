import { getSolutions, WPSolution } from '@/services/wordpress';
import Link from 'next/link';
import Image from 'next/image';
import PageHeader from '@/components/layout/PageHeader';

export const metadata = {
  title: "AI & ML Solutions | Identifyyou",
  description: "Leveraging AI and Machine Learning to drive next-generation innovation.",
};
...
export default async function AiMlPage() {
  ...
  try {
    cards = await getSolutions('ai-ml');
  } catch (err) {
    console.error("Error loading AI-ML solutions:", err);
    error = "Failed to load AI-ML solutions dynamically.";
  }

  return (
    <div className="flex flex-col min-h-screen bg-black">
      <PageHeader title="AI & ML Solutions" />
                  ...
                  <div className="bg-white border border-[#eeeeee] rounded-none overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 hover:border-primary h-full flex flex-col">
                    ...
                    <div className="p-8 flex flex-col flex-grow">
                      ...
                      <div className="flex items-center text-primary font-bold mt-auto group-hover:text-secondary transition-colors">
                        <span className="uppercase tracking-wider text-sm">{card.buttonText || 'View Details'}</span>
